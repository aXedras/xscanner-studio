...
line("  npm run check:fast      # architecture guards + duplication + eslint + type-check + unit tests");
line("");
line("Pre-commit gate:");
line("  npm run check:pre-commit # format + duplication + architecture guards + i18n + lint + types + unit tests");
line("");
line("Quality gate (still no API server required):");
line("  npm run check:all       # format + architecture guards + lint + types + i18n + db types + build + unit tests");
line("");
