...
def run_benchmark(args) -> int:
    ...
    # Legacy PaddleOCR + Llama hybrid (optional; enable explicitly via filter)
    if strategy_filter and "hybrid" in strategy_filter:
        try:
            base_url = os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434")
            strategies.append(PaddleLlamaHybridStrategy(base_url=base_url))
            print("✓ PaddleOCR + Llama Hybrid")
        except Exception as e:
            print(f"⚠️  PaddleOCR + Llama Hybrid skipped: {e}")
